// src/routes/birth-chart.routes.ts
...
const getBirthChart = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const requestData: BirthChartRequest = {
      day,
      month,
      year,
      hour,
    ...
    res.status(500).json({ message: 'Failed to get birth chart details' });
  }
};

// Reuse the same authMiddleware from gem routes
