...
const getPlanetPositions = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const requestData: PlanetRequest = {
      day,
      month,
      year,
      hour,
    ...
    console.error('Planet positions error:', error);
    res.status(500).json({ message: 'Failed to get planet positions' });
  }
};
